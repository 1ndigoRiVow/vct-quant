...
针对 VCT CN（无畏契约中国赛区）的电竞量化预测与虎扑社区选手股模拟交易策略系统
...
| 建模 | `models/` | Glicko-2 评分 + 情绪定价 + 价值残差 + walk-forward 回测 + 选手多维画像 + 比赛蒙特卡洛模拟器 |
...
`models/match_simulator.py`：FPS 回合制蒙特卡洛引擎，输出**赛前胜率预测 p̂**——
这是双层预测模型第一层（内在价值）的关键补强，也是第二层"惊喜度 s = 实际结果 − p̂"的核心输入。

引擎流程（借鉴 esports-manager 事件驱动思路，改造为 VCT 回合制）：
1. 队伍实力 = Glicko-2 队员均值（按 rd 采样体现评分不确定性）+ 该图历史胜率修正
2. 逻辑斯蒂映射为单回合胜率 → 13 胜制逐回合模拟（领先≥2，12:12 进加时）
3. 蒙特卡洛重复 N 次 → p̂ + 95% 置信区间

`evaluate_calibration` 用历史比赛回测排序能力，输出 Brier / Log-Loss / 胜负命中率；
报告与仪表盘含校准板块（预测 p̂ vs 实际结果散点）。
注：离线合成数据的比赛结果近乎随机，校准指标仅用于验证模拟器逻辑；接入真实数据后指标才有参考意义。
